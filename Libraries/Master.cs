...
namespace Libraries
{
    public static class Master
    {
        ...
        public static byte[] Decompress(byte[] RomData, uint Start, CompressionType Type)
        {
            ...
            {
                CompressionType.LZ1 => new Lz1AndLz2().Decompress(RomData, Start, Type),
                CompressionType.LZ2 => new Lz1AndLz2().Decompress(RomData, Start, Type),
                CompressionType.MarioPicross => new MarioPicross().Decompress(RomData, Start),
        ...
        public static byte[] Compress(byte[] Uncompresed, CompressionType Type)
        {
            ...
            {
                CompressionType.LZ1 => (new Lz1AndLz2()).Compress(Uncompresed, Type),
                CompressionType.LZ2 => (new Lz1AndLz2()).Compress(Uncompresed, Type),
                CompressionType.MarioPicross => (new MarioPicross()).Compress(Uncompresed),
                _ => throw new Exception("Not a valid compression type."),
            };
            return Compressed;
        }
